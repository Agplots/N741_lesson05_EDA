...
* `car` [which also loads the `carData` package]
* `tidyverse` - mainly using `dplyr` and `ggplot2`
* `janitor` _(optional)_
* `Rcmdr` _(short demo if time)_
* `tableone` _(optional)_

## Rmarkdown initial setup options

Create an initial R chunk with `knitr` options to control what is shown or not shown in your final output. Three options are useful to set to FALSE for the final version. These options can be kept as TRUE when debugging initially.
...
* `error` - set to FALSE to hide error messages in final document

Learn more about `knitr` at [https://yihui.name/knitr/options/](https://yihui.name/knitr/options/). Another option to consider is `include = FALSE` which will run the code but produce no output in the document for that code chunk.

NOTE: In this first code chunk, it is a good idea to load all the packages needed for all of the code run in this RMD (R markdown) document.
...
The names of your code chunks will show up in the "Rmarkdown" TAB (next to Console) when the document is "knitted".

## Working with `Leinhardt` dataset in `carData` package

The `carData` package has many datasets built into it. One of these datasets is the `Leinhardt` dataset. Type `?carData::Leinhardt` at the console to see help details on this dataset. This dataset has 105 rows and 4 columns with observations from nations of the world around 1970. The 4 columns are:
...
Load the dataset and take a look at it. We can access the dataset directly using the `View()` command and by referencing the dataset using the `package::dataset` syntax. In console type `View(carData::Leinhardt)`, which will open a data viewer window (usually at the top left side of the RStudio IDE).
...
_Creating a copy of the dataset is usually a good idea to be sure you don't overwrite the original data source._

```{r loadData}
dataLeinhardt <- carData::Leinhardt
```

## Dataset object

Look at the type of object this dataset is (using the `class()` function) and the structure of the data (using the `str()` function) - the types of variables in the dataset.
...
## [OPTIONAL] `tableone` package

Another good package to explore is the `tableone` package. Learn more at [https://cran.r-project.org/web/packages/tableone/](https://cran.r-project.org/web/packages/tableone/), review the vignettes and see their Github repo [https://github.com/kaz-yos/tableone](https://github.com/kaz-yos/tableone).

We can get a summary table of the overall statistics using the `tableone::CreateTableOne(data = xxxx)` function.

### Summary Stats Table Overall

```{r tableoneCreate}
# load tableone package - done in 1st code chunk above
# library(tableone)

# simple summary stats
tableone::CreateTableOne(data = dataLeinhardt)
```
...
tab1 <- tableone::CreateTableOne(data = dataLeinhardt)
...
### Table by Group with stat tests for group differences

```{r tableoneGroup}
# get stats by group
tab2 <- CreateTableOne(strata = "region" , 
                       data = dataLeinhardt)
tab2
```

### Nicer table by group with stat tests

```{r tableoneGrouptests}
ptab2 <- print(tab2)
class(ptab2)
